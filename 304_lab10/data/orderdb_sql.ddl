DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Countries');
INSERT INTO category(categoryName) VALUES ('Patterns');
INSERT INTO category(categoryName) VALUES ('Colours');
INSERT INTO category(categoryName) VALUES ('TV Shows');
INSERT INTO category(categoryName) VALUES ('Video Games');

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Bojack Horseman', 4, 'The best horse'16.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Canada', 1, 'The best country',9.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cats & Goats', 2, 'The GOAT',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Family Guy', 4, 'Peter Griffin',13.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gen-Z Yellow', 3, 'A bright and vibrant yellow',9.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Green Camo', 2, 'We will guarantee that you will lose your clicker',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('League of Legends', 5, 'A toxic skin',12.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Louis Vuitton', 2, 'a Gucci iSticker',15.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Millennial Pink', 3, 'Pastel pink, but we call it Millennial',11.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Minecraft', 5, 'You can barely see the pixels!',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Overwatch', 5, 'Reapers pretty edgy',16.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Pokemon', 5, 'How can pikachu make a baby laugh? By playing pika-boo!',14.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Red Camo', 2, 'Bloody well hidden',17.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Spongebob', 4, 'Your everyday multi-use sponge',28.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Yoshi', 5, 'dinosaur from Nintendo.',18.00);


  
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Ramon', 'Lawrence', 'ramon.lawrence@ubc.ca', '555-555-0000', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'ramon' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Nick', 'Nad', 'beautyboi99@hotmail.com', '555-555-0001', '8239 Pizza Hut Circle', 'Saskatoon', 'SA', 'T8E 6J8', 'Canada', 'nad' , 'banana');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Gabe', 'Tiny', 'tiny_gabe@gmail.com', '555-555-0002', '4234 Frog Drive', 'Cranbrook', 'BC', 'L8T 9B5', 'Canada', 'rex' , 'dino');

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 91.70)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 18)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 31);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 106.75)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 21.35);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 140)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 30);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 327.85)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 10)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 40)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 23.25)

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-15 10:25:55', 277.40)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 4, 21.35)
I

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/bojack.png' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/canada leaf.png' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/cats&goats.png' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/family guy.png' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/gen-z yellow.png' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/green camo.png' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/lol.png' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/LV.png' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/millennial pink.png' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/minecraft.png' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/overwatch.png' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/pika.png' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/red camo.png' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/spongebob.png' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/yoshi.png' WHERE ProductId = 15;

-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;